...
public class Parqueadero {
    ...
    public String metodo1() {
        String placaMasTiempo = "";
        int horaLlegada = 40;

        for (Puesto puesto: puestos) {
            if (puesto.estaOcupado()){
                Carro carro = puesto.darCarro();
                if(carro.darHoraLlegada() < horaLlegada) {
                    horaLlegada = carro.darHoraLlegada();
                    placaMasTiempo = carro.darPlaca();
                }
            }
        }
        return "La placa que lleva mas tiempo en el parqueadero es " + placaMasTiempo;
    }
